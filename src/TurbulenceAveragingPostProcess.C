  ...
  if (y_average) {    
    ...
    if (y_specs) {
      for (size_t ispec = 0; ispec < y_specs->size(); ++ispec) {
        ...
        // ================================Q criterion====================================================
        get_if_present(y_spec, "compute_Qcriterion", avInfo->computeQcriterion_, avInfo->computeQcriterion_);
        // ================================Q criterion====================================================
...
TurbulenceAveragingPostProcessing::setup()
{
  ...
  for (size_t k = 0; k < averageInfoVec_.size(); ++k ) {
    ...
    for ( size_t itarget = 0; itarget < avInfo->targetNames_.size(); ++itarget ) {
      ...
      const int vortSize = realm_.spatialDimension_;  // == 3 ? 3 : 2;
      if ( avInfo->computeVorticity_ ) {
              // hack a name; the name is not tied to the average info name
              const std::string vorticityName = "vorticityField";
      //        const int sizeOfField = 1;
      ...
      // =====================Q criterion======================================
            ...
            if ( avInfo->computeQcriterion_ ) {
                    // hack a name; the name is not tied to the average info name
                    const std::string QcritName = "QcriterionField";
                    const int sizeOfField = 1;
  ...
  const AveragingInfo *avInfo)
{
  ...
  // ================================vorticity====================================================
  if ( avInfo->computeVorticity_ ) {
      NaluEnv::self().naluOutputP0() << "Vorticity will be computed; add vorticityField to output"<< std::endl;
    }
  // ================================vorticity====================================================

  // ================================Q criterion====================================================
    if ( avInfo->computeQcriterion_ ) {
        NaluEnv::self().naluOutputP0() << "Q criterion will be computed; add QcriterionField to output"<< std::endl;
      }
  // ================================Q criterion====================================================
...
TurbulenceAveragingPostProcessing::execute()
{
  ...
  for (size_t k = 0; k < averageInfoVec_.size(); ++k ) {
    ...
    // ============Q criterion=========================
        if ( avInfo->computeQcriterion_ ) {
              compute_Qcriterion(avInfo->name_, s_all_nodes);
            }
    // ====================Q criterion=========================
  ...
  stk::mesh::Selector s_all_nodes)
{
  ...
  const std::string resolvedVortName = "vorticityField";

  stk::mesh::FieldBase *dudx_ = metaData.get_field(stk::topology::NODE_RANK, "dudx");
  stk::mesh::FieldBase *resolvedVort = metaData.get_field(stk::topology::NODE_RANK, resolvedVortName);
  ...
  for ( stk::mesh::BucketVector::const_iterator ib = node_buckets_vort.begin();
   ...
   const double * du = (double*)stk::mesh::field_data(*dudx_, b);
   double *vorticity_ = (double*)stk::mesh::field_data(*resolvedVort,b);
   ...
   //     const double *dudx = stk::mesh::field_data(*dudx_, b[k] );
      ...
      //const double wx = du[offSet*k + 7] - du[offSet*k + 5];
     // const double wy = du[offSet*k + 2] - du[offSet*k + 6];
     // const double wz = du[offSet*k + 3] - du[offSet*k + 1];
         // 	if(nDim == 2)
         //  	{
         //  		vorticity_[k*nDim ] = 0.0;
         //   		vorticity_[k*nDim + 1] = 0.0;
         //  		vorticity_[k*nDim + 2] = wz;

         //     }
    	//vorticity_[k*nDim ] = wx;
    	//vorticity_[k*nDim + 1] = wy;
    	//vorticity_[k*nDim + 2] = wz;


    	// might have to initialize vorticity_ to zeroes for 2-D case
...
//--------------------------------------------------------------------------
void
TurbulenceAveragingPostProcessing::compute_Qcriterion(
  ...
  stk::mesh::Selector s_all_nodes)
{
  ...
  const std::string resolvedQcritName = "QcriterionField";

  stk::mesh::FieldBase *dudx_ = metaData.get_field(stk::topology::NODE_RANK, "dudx");
  stk::mesh::FieldBase *resolvedQcrit = metaData.get_field(stk::topology::NODE_RANK, resolvedQcritName);
...
// const double * du     =  (double*)stk::mesh::field_data(*dudx_, b);
  // const double * du = (double*)stk::mesh::field_data(*dudx_, b);
   double *Qcriterion_ = (double*)stk::mesh::field_data(*resolvedQcrit,b);
   ...
   const int offSet = nDim*nDim;
    ...
    for ( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k ) {
    	...
    	for (int i = 0; i < nDim; ++i){
         ...
         Qcriterion_[k] = 0.5*(Omegaij - Sij);

    }
  }
  const double inv_maxQ = 1.0/maxQ;
  for ( stk::mesh::BucketVector::const_iterator ib = node_buckets_vort.begin();
		  ib != node_buckets_vort.end() ; ++ib ){
	  stk::mesh::Bucket & b = **ib ;
	  const stk::mesh::Bucket::size_type length = b.size() ;
	  ...
	  for( stk::mesh::Bucket::size_type k = 0 ; k < length ; ++k){
		  Qcriterion_[k] = Qcriterion_[k]*inv_maxQ ;
